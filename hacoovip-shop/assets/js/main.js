(() => {
  ...
  const PREFIX_TO_LANG = { en:"en", zh:"zh-CN", es:"es", fr:"fr", de:"de", it:"it", pt:"pt", pl:"pl", nl:"nl" };
  ...
  function getPathInfo(){
    ...
    const first = parts[0] || "";
    const isPrefixed = Object.prototype.hasOwnProperty.call(PREFIX_TO_LANG, first);
    const currentLang = isPrefixed ? PREFIX_TO_LANG[first] : "en";
    const slug = isPrefixed ? (parts.slice(1).join("/") || "") : (parts.join("/") || "");
    return { currentLang, slug, isPrefixed };
  }

  function buildLangUrl(targetLang){
    const { slug } = getPathInfo();
    const clean = slug === "index.html" ? "" : slug;
    const target = LANGS[targetLang] ? targetLang : "en";
    if (target === "en") return clean ? `/${clean}` : "/";
    const prefix = LANGS[target].prefix;
    if (LOCALIZED_PAGES.has(clean)) return clean ? `/${prefix}/${clean}` : `/${prefix}/`;
    if (ARTICLE_PAGES.has(clean)) return `/${prefix}/seo-articles.html`;
    return `/${prefix}/`;
  }

  function getAssetBase(){ return getPathInfo().isPrefixed ? "../" : ""; }
  ...
  function renderLocalizedPage(){
    ...
    if(page === "categories"){
      ...
      body = cardsHtml([{t:lang==="zh-CN"?"鞋":"Shoes",p:lang==="zh-CN"?"运动鞋和鞋类分类入口。":"Sneaker and footwear category entry.",href:"https://www.cnfanshp.com/shoes/?utm_source=hacoovip.shop&utm_medium=referral&utm_campaign=localized_cat_shoes"},{t:lang==="zh-CN"?"连帽衫":"Hoodies",p:lang==="zh-CN"?"卫衣、毛衣和层搭单品。":"Sweatshirts, hoodies and layering pieces.",href:"https://www.cnfanshp.com/hoodies-sweaters/?utm_source=hacoovip.shop&utm_medium=referral&utm_campaign=localized_cat_hoodies"},{t:lang==="zh-CN"?"夹克":"Jackets",p:lang==="zh-CN"?"外套和季节性单品入口。":"Outerwear and seasonal jacket finds.",href:"https://www.cnfanshp.com/jackets/?utm_source=hacoovip.shop&utm_medium=referral&utm_campaign=localized_cat_jackets"}], d.read);
    }
    if(page === "trending"){
      title = d.trendTitle; lead = d.trendLead;
      body = cardsHtml([{t:"Running / Streetwear Sneaker",p:lang==="zh-CN"?"打开匹配的产品详情页。":"Open the matching product-detail page.",href:"https://www.cnfanshp.com/AllProducts/6045.html?utm_source=hacoovip.shop&utm_medium=referral&utm_campaign=localized_trend_shoes"},{t:"Sweatshirt",p:lang==="zh-CN"?"检查图片和材质线索。":"Inspect photos and material cues.",href:"https://www.cnfanshp.com/AllProducts/5974.html?utm_source=hacoovip.shop&utm_medium=referral&utm_campaign=localized_trend_sweatshirt"},{t:"Jacket",p:lang==="zh-CN"?"查看颜色、细节和质检说明。":"Review colors, details and QC notes.",href:"https://www.cnfanshp.com/AllProducts/5919.html?utm_source=hacoovip.shop&utm_medium=referral&utm_campaign=localized_trend_jacket"}], d.read);
    }
    if(page === "seo"){
      title = d.seoTitle; lead = d.seoLead;
      body = cardsHtml([{t:"Hacoo Spreadsheet 2026 Guide",p:lang==="zh-CN"?"把产品表格当作研究地图。":"Use product lists as research maps.",href:"../hacoo-spreadsheet-2026-guide.html"},{t:"Hacoo QC Photos Guide",p:lang==="zh-CN"?"检查图片和细节后再继续。":"Inspect photos before action.",href:"../hacoo-qc-photos-guide.html"},{t:"Reverse Image Search Workflow",p:lang==="zh-CN"?"验证图片线索和产品路径。":"Verify image-led product leads.",href:"../hacoo-reverse-image-search-workflow.html"}], d.read);
    }
    if(page === "how"){
      title = d.howTitle; lead = d.howLead;
      body = cardsHtml([{t:d.products,p:lang==="zh-CN"?"先从产品图进入详情页。":"Start from product covers and detail pages.",href:"trending.html"},{t:d.categories,p:lang==="zh-CN"?"再按类目继续浏览。":"Continue through product categories.",href:"categories.html"},{t:d.qc,p:lang==="zh-CN"?"继续前先检查 QC 线索。":"Check QC signals before action.",href:"quality-control-guide.html"}], d.read);
    }
    if(page === "qc"){
      title = d.qcTitle; lead = d.qcLead;
      body = cardsHtml([{t:lang==="zh-CN"?"检查首图":"Check cover image",p:lang==="zh-CN"?"确认图片、标题和跳转目标是否匹配。":"Confirm image, title and destination match.",href:"quality-control-guide.html"},{t:lang==="zh-CN"?"查看细节":"Review details",p:lang==="zh-CN"?"查看材质、走线、颜色和尺码。":"Look at material, stitching, color and sizing.",href:"quality-control-guide.html"},{t:d.seo,p:lang==="zh-CN"?"打开更完整的研究文章。":"Open the deeper research workflow.",href:"seo-articles.html"}], d.read);
    }
    if(page === "faq"){
      title = d.faqTitle; lead = d.faqLead;
      body = cardsHtml([{t:lang==="zh-CN"?"这是官网吗？":"Is this official?",p:lang==="zh-CN"?"不是，这是独立指南。":"No. It is an independent guide, not official Hacoo.",href:"faq.html"},{t:lang==="zh-CN"?"销售产品吗？":"Does it sell products?",p:lang==="zh-CN"?"不销售，只提供研究入口。":"No. It links to product research pages and categories.",href:"faq.html"},{t:lang==="zh-CN"?"为什么先看 QC？":"Why use QC first?",p:lang==="zh-CN"?"单张首图不足以支持决定。":"A strong cover image is not enough for a safe decision.",href:"quality-control-guide.html"}], d.read);
    }
  ...
  function setupLanguageSwitcher(){
    ...
    if(currentLabel) currentLabel.textContent = LANGS[currentLang]?.label || "English";
    ...
    document.querySelectorAll("[data-lang]").forEach(btn => {
      ...
      btn.addEventListener("click", e => { e.preventDefault(); location.href = buildLangUrl(btn.dataset.lang); });
    });
  }
  ...
  document.addEventListener("DOMContentLoaded", () => { renderLocalizedPage(); setupMenu(); setupLanguageSwitcher(); setupStructuredData(); });
})();
