...
for (const article of allArticles) {
  ...
  if (!match) { problems.push(`${article.slug}: missing long-form article marker`); continue; }
...
if (problems.length) { console.error(`Article validation failed:\n- ${problems.join("\n- ")}`); process.exit(1); }
console.log(`Article validation passed for ${allArticles.length} research articles.`);
