...
export default function FAQPage(){const schema={"@context":"https://schema.org","@type":"FAQPage",mainEntity:faqs.map(([q,a])=>({"@type":"Question",name:q,acceptedAnswer:{"@type":"Answer",text:a}}))};return <><StructuredData data={schema}/><BreadcrumbData path="/faq" items={[{ name: "Home", path: "/" }, { name: "FAQ", path: "/faq" }]}/><section className="page-hero simple-hero"><div className="wrap"><span className="section-label">Frequently asked</span><h1>Direct answers.<br/><em>No guesswork.</em></h1><p>What this independent site does, where links go and which information still needs to be verified on the live destination.</p></div></section><section className="section wrap faq-page"><div className="faq-list">{faqs.map(([q,a])=><details key={q}><summary>{q}<span>+</span></summary><p>{a}</p></details>)}</div><p>For a parcel that arrived without every ordered item, follow the <Link href="/articles/hacoo-order-missing-item-split-shipment/">Hacoo missing-item and split-shipment decision path</Link>.</p></section></>}
