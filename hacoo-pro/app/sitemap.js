...
export default function sitemap() {
  ...
  const research = [...priorityArticles, ...articles];
  const latestArticleUpdate = new Date(Math.max(...research.map((article) => new Date(article.modified).getTime())));
  const localizedCore = ["/", "/spreadsheet", "/categories", "/products", "/guides", "/faq", "/about"].flatMap((path) => LOCALES.map((locale) => ({ url: canonicalUrl(path, locale), lastModified: path === "/" && locale === "en" ? latestArticleUpdate : now, changeFrequency: path === "/" ? "weekly" : "monthly", priority: path === "/" ? 1 : 0.7 })));
  const localizedCategories = categories.flatMap((category) => LOCALES.map((locale) => ({ url: canonicalUrl(`/categories/${category.slug}`, locale), lastModified: now, changeFrequency: "weekly", priority: 0.8 })));
  const localizedGuides = guides.flatMap((guide) => LOCALES.map((locale) => ({ url: canonicalUrl(`/guides/${guide.slug}`, locale), lastModified: now, changeFrequency: "monthly", priority: 0.75 })));
