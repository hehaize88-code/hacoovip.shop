...
export function LocalizedCategory({ locale, slug }) {
  ...
  const schema = { "@context": "https://schema.org", "@graph": [{ "@type": "CollectionPage", name: category.name, description: category.description, url: absoluteLocalizedUrl(`/categories/${slug}`,locale), inLanguage: locale, dateModified: CATALOG_REVIEW.iso }, { "@type": "ItemList", name: `${category.name} product reference`, itemListElement: [{ "@type": "ListItem", position: 1, name: featuredProduct.name, url: verifiedListing }] }, { "@type": "FAQPage", inLanguage: locale, mainEntity: faqItems.map(([question,answer])=>({ "@type": "Question", name: question, acceptedAnswer: { "@type": "Answer", text: answer } })) }] };
  return <div className="localized-shell" lang={locale}><StructuredData data={schema}/><section className="category-hero"><div className="category-hero-image"><img src={category.image} alt={category.name}/></div><div className="category-hero-copy"><span className="section-label">{category.eyebrow} / {page.label}</span><h1>{category.name}<br/><em>{copy.nav.spreadsheet}.</em></h1><p>{category.description}</p><a className="button primary" href={`${DESTINATION}${category.destination}`} target="_blank" rel="noopener noreferrer">{page.live} {category.name} <Arrow/></a></div></section>
    ...
    <section className="verified-product-section"><div className="wrap"><div className="section-heading compact"><div><span className="section-label">{depth.category.productLabel}</span><h2>{depth.category.productTitle}</h2></div><p>{depth.category.reviewed}</p></div><article className="verified-product"><div className="verified-product-image"><img src={featuredProduct.image} alt={featuredProduct.name}/><span>{CATALOG_REVIEW.label}</span></div><div className="verified-product-copy"><div className="verified-product-status"><span></span>#{featuredProduct.listingId}</div><h3>{featuredProduct.name}</h3><p>{featuredProduct.focus}</p><dl><div><dt>CNFansHP</dt><dd>{featuredProduct.catalogLabel}</dd></div><div><dt>{depth.category.updateLabel}</dt><dd>{CATALOG_REVIEW.label}</dd></div></dl><div className="hero-actions"><a className="button primary" href={verifiedListing} target="_blank" rel="noopener noreferrer">{depth.category.openListing} <Arrow/></a><a className="button quiet" href={liveSearch} target="_blank" rel="noopener noreferrer">{depth.category.fallback}</a></div><p className="verified-product-disclosure">{depth.category.disclosure}</p></div></article></div></section>
