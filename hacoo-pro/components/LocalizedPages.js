...
import { createBreadcrumbList, getGuideArticleImage, ORGANIZATION_ID, pageUrl, WEBSITE_ID } from "@/app/schema";
...
export function LocalizedCategory({ locale, slug }) {
  ...
  const worksheet = slug === "electronics" ? depth.category.specificationTable : depth.category;
  ...
  const categoryUrl = pageUrl(`/categories/${slug}`, locale);
  ...
  const breadcrumb = createBreadcrumbList({ locale, path: `/categories/${slug}`, items: [{ name: "Hacoo Pro", path: "/" }, { name: copy.nav.categories, path: "/categories" }, { name: category.name, path: `/categories/${slug}` }] });
  const schema = { "@context": "https://schema.org", "@graph": [{ "@type": "CollectionPage", "@id": `${categoryUrl}#webpage`, name: category.name, description: category.description, url: categoryUrl, inLanguage: locale, dateModified: CATALOG_REVIEW.iso, mainEntity: { "@id": productReferenceUrl }, breadcrumb: { "@id": breadcrumb["@id"] }, isPartOf: { "@id": WEBSITE_ID } }, { "@type": "ItemList", name: `${category.name} product reference`, itemListElement: [{ "@type": "ListItem", position: 1, name: featuredProduct.name, url: productReferenceUrl }] }, { "@type": "FAQPage", inLanguage: locale, mainEntity: faqItems.map(([question,answer])=>({ "@type": "Question", name: question, acceptedAnswer: { "@type": "Answer", text: answer } })) }, breadcrumb] };
  return <div className="localized-shell" lang={locale}><StructuredData data={schema}/><section className="category-hero"><div className="category-hero-image"><img src={category.image} alt={category.name}/></div><div className="category-hero-copy"><span className="section-label">{category.eyebrow} / {page.label}</span><h1>{category.name}<br/><em>{copy.nav.spreadsheet}.</em></h1><p>{category.description}</p><a className="button primary" href={`${DESTINATION}${category.destination}`} target="_blank" rel="noopener noreferrer">{page.live} {category.name} <Arrow/></a></div></section>
    ...
    <section className="section wrap"><div className="section-heading"><div><span className="section-label">{worksheet.tableLabel}</span><h2>{worksheet.tableTitle}</h2></div></div><div className="research-table-wrap"><table className="research-table"><thead><tr>{depth.category.tableHeaders.map((header)=><th key={header}>{header}</th>)}</tr></thead><tbody>{checks.map((check)=><tr key={check}><th>{check}</th><td>{worksheet.tableRecord}</td><td>{worksheet.tableEvidence}</td></tr>)}</tbody></table></div></section>
    <section className="update-section"><div className="wrap update-layout"><div><span className="section-label">{depth.category.updateLabel}</span><h2>{depth.category.updateTitle}</h2></div><ol className="update-log"><li><time dateTime={CATALOG_REVIEW.iso}>{CATALOG_REVIEW.label}</time><div><h3>#{featuredProduct.listingId}</h3><p>{worksheet.updateText}</p></div></li></ol></div></section>
...
export function LocalizedGuide({ locale, slug }) {
  ...
  const articleUrl = pageUrl(`/guides/${slug}`, locale);
  const image = getGuideArticleImage(slug, guide.title);
  const imageId = `${articleUrl}#primaryimage`;
  const breadcrumb = createBreadcrumbList({ locale, path: `/guides/${slug}`, items: [{ name: "Hacoo Pro", path: "/" }, { name: copy.nav.guides, path: "/guides" }, { name: guide.title, path: `/guides/${slug}` }] });
  const schema = { "@context": "https://schema.org", "@graph": [
    { "@type": "Article", "@id": `${articleUrl}#article`, headline: guide.title, description: guide.short, image: { "@id": imageId }, mainEntityOfPage: { "@id": `${articleUrl}#webpage` }, inLanguage: locale, author: { "@type": "Organization", name: "Hacoo Pro Editorial", url: `${SITE_URL}/about/` }, publisher: { "@id": ORGANIZATION_ID }, datePublished: "2026-07-14", dateModified: "2026-07-16" },
...
export function localizedPageMetadata(locale, pathname, title, description, options = {}) {
  ...
  else if (locale === "es" && pathname === "/guides/size-guide") seoTitle = "Guía de tallas Hacoo 2026: ropa y calzado";
