...
export const metadata: Metadata = { title:"Hipobuy Product Categories 2026: Shoes, Clothing & More", description:"Browse focused Hipobuy spreadsheet categories, checked product rows and verification checklists.", alternates:{canonical:"/categories/"} };
const cats = [
...
export default function CategoriesPage(){ return <SiteChrome><main className="inner-main"><InnerHero eyebrow="Twelve focused routes · six verified landing pages" title="Browse Hipobuy product categories." intro="Start with product intent instead of scrolling a mixed list. The original catalogue routes remain available below; six independent landing pages now add checked rows, dated price references and category-specific verification steps."/><section className="category-image-grid inner-category-images">{cats.map(c=><a href={c.href} target="_blank" rel="noopener noreferrer" key={c.name} aria-label={`Open ${c.name} on cnfanshp.com`}><span className="category-product"><img src={c.image} alt="" loading="lazy" /></span><h3>{c.name}</h3><b aria-hidden="true">↗</b></a>)}</section><section className="verified-category-links" aria-labelledby="verified-categories-title"><div><span>INDEXABLE CATEGORY PAGES</span><h2 id="verified-categories-title">Open a checked category before the live listing.</h2><p>These pages keep product evidence on this site while preserving every exact product button’s original destination.</p></div><nav>{catalogCategories.map((category,index)=><a href={`/categories/${category.slug}/`} key={category.slug}><small>0{index+1}</small><strong>{category.name}</strong><span>10 checked rows →</span></a>)}</nav></section><section className="decision-note"><span>DIRECT ROUTES</span><h2>Less wandering. More checking.</h2><p>Opening a focused category is only the first step. Confirm product options and inspect available warehouse photos before approving an item.</p></section></main></SiteChrome> }
