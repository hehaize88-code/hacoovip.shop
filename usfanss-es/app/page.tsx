  ...
  return <SiteShell>
    <section className="hero">
      ...
      <div className="hero-gallery" aria-label="Product selection">
        <a className="hero-card hero-main" href="https://cnfanshp.com/AllProducts/5756.html" target="_blank" rel="noreferrer"><img src={products[0].image} alt={products[0].name}/><span>{d.drop}</span><div><small>{d.hoodie}</small><b>{d.layers}</b></div></a>
        <a className="hero-card hero-mini one" href="https://cnfanshp.com/AllProducts/5964.html" target="_blank" rel="noreferrer"><img src={products[1].image} alt={products[1].name}/><b>{d.trending} ↗</b></a>
        <a className="hero-card hero-mini two" href="https://cnfanshp.com/AllProducts/5919.html" target="_blank" rel="noreferrer"><img src={products[2].image} alt={products[2].name}/><b>{d.newest} ↗</b></a>
    ...
    <section className="discover">
      ...
      <div className="product-grid">{products.map((item,index) => <a className={`product ${item.tone} p${index+1}`} key={item.id} href={`https://cnfanshp.com/AllProducts/${item.id}.html`} target="_blank" rel="noreferrer">
        <div className="product-photo"><img src={item.image} alt={item.name} loading={index > 2 ? "lazy" : "eager"}/><span>0{index+1}</span><button tabIndex={-1} aria-hidden="true">↗</button></div>
        <div className="product-copy"><small>{d.categoryNames[index]}</small><h3>{item.name}</h3><p>{d.productOpen}</p></div>
      </a>)}</div>
      <a className="collection-link" href="https://cnfanshp.com/AllProducts/" target="_blank" rel="noreferrer"><span>{d.collection}</span><b>{d.collectionDesc}</b><i>↗</i></a>
    </section>
