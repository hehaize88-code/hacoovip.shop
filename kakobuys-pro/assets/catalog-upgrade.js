...
const KB_COPY = {
  en: {
    ...
    homeIntro: "Every card opens a Kakobuys.pro detail page first. The detail page records the source title, item ID, first image and reference price before you continue.",
  ...
  pl: {
    ...
    homeIntro: "Każda karta najpierw otwiera stronę szczegółów Kakobuys.pro z nazwą źródłową, ID, pierwszym zdjęciem i ceną orientacyjną.",
  ...
  de: {
    ...
    homeIntro: "Jede Karte öffnet zuerst eine Kakobuys.pro-Detailseite mit Quelltitel, Artikel-ID, erstem Bild und Referenzpreis.",
  ...
  fr: {
    ...
    homeIntro: "Chaque carte ouvre d’abord une fiche Kakobuys.pro indiquant le titre source, l’ID, la première image et un prix de référence.",
  ...
  it: {
    ...
    homeIntro: "Ogni scheda apre prima una pagina Kakobuys.pro con titolo originale, ID, prima immagine e prezzo di riferimento.",
...
function kbLanguage() {
  ...
  return KB_LANGS.includes(fromUrl) ? fromUrl : "en";
}
...
function kbAddItemList(products, catalog) {
  ...
  script.textContent = JSON.stringify({
    ...
    itemListElement: products.map((product, index) => ({
      "@type": "ListItem",
      position: index + 1,
      url: `https://kakobuys.pro/products/${product.page}/`,
      item: {
        "@type": "Product",
        name: product.name,
        image: product.image,
...
function kbRenderProduct(catalog, language) {
  ...
  const canonical = `https://kakobuys.pro/products/${product.page}/`;
  document.documentElement.lang = language;
  document.title = `${product.name} — Product ID ${product.id} | Kakobuys.pro`;
  ...
  kbSetMeta('meta[property="og:title"]', `${product.name} | Kakobuys.pro`);
