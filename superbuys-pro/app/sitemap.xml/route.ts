...
const posts=articles.map(article=>article.slug);
export async function GET(){
  ...
  const xml=`<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${urls.map((url,i)=>{const article=articles.find(item=>url.includes(`/articles/${item.slug}/`));return `<url><loc>${url}</loc><lastmod>${article?.date||CATALOG_UPDATED}</lastmod><changefreq>${i%12===0?"daily":"weekly"}</changefreq><priority>${i%12===0?"1.0":"0.8"}</priority></url>`}).join("")}</urlset>`;
