import { ArticleSlug, categories, copy, Lang, languages, localizedPath, products, RouteKey } from "./site-data";
...
function Header({ lang, routePath }: { lang: Lang; routePath: string }) {
  ...
  const nav = [
    ...
    ["shipping", t.nav.shipping], ["articles", t.nav.articles], ["faq", t.nav.faq],
  ];
  ...
  return (
    <header className="site-header">
      ...
      <div className="header-actions">
        ...
        <a className="open-all" href={ALL_PRODUCTS} target="_blank" rel="noreferrer">{t.openAll}</a>
        <details className="mobile-menu">
          <summary aria-label="Menu">MENU</summary>
          <div>{nav.map(([path, label]) => <a key={path} href={localizedPath(lang, path)}>{label}</a>)}<a className="mobile-open-all" href={ALL_PRODUCTS} target="_blank" rel="noreferrer">{t.openAll}</a></div>
        </details>
      </div>
    </header>
  );
...
function PageHero({ eyebrow, title, text }: { eyebrow: string; title: string; text: string }) {
  return <section className="page-hero"><span>{eyebrow}</span><h1>{title}</h1><p>{text}</p></section>;
}
...
function ProductGrid({ lang }: { lang: Lang }) {
  ...
  return <div className="product-grid">{products.map((product, index) => (
    <a className="product-card" href={product.href} target="_blank" rel="noreferrer" key={product.href}>
      {/* Local, verified catalog assets are served directly to avoid transforming product evidence. */}
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <div className="product-image"><img src={product.image} alt={labels[index].title} /><span>{product.price}</span></div>
...
function CategoryGrid({ lang, limit }: { lang: Lang; limit?: number }) {
  ...
  return <div className="category-grid">{categories.slice(0, limit).map((category, index) => (
    <a href={category.href} target="_blank" rel="noreferrer" key={category.href}>
...
function Home({ lang }: { lang: Lang }) {
  ...
  return <>
    <section className="home-hero">
      ...
      <div className="trending-panel"><div><span>{t.trending}</span><a href={localizedPath(lang, "categories")}>{t.viewAll} →</a></div>{categories.slice(0, 4).map((category, index) => <a key={category.href} href={category.href} target="_blank" rel="noreferrer"><span>{category.icon}</span><b>{localizedCatalog[lang].categories[index]}</b><i>↗</i></a>)}</div>
    </section>
    ...
    <section className="section articles-home"><div className="section-heading"><span>{t.articlesLabel}</span><h2>{t.articlesTitle}</h2><p>{t.articlesText}</p></div><ArticleCards lang={lang} /><a className="text-link" href={localizedPath(lang, "articles")}>{t.articleIndexText} →</a></section>
...
function ArticlePage({ lang, articleSlug }: { lang: Lang; articleSlug: ArticleSlug }) {
  ...
  return <article className="article-page">
    ...
    <header><a href={localizedPath(lang, "articles")}>← {t.nav.articles}</a><span>{article.category} · {article.read}</span><h1>{article.title}</h1><p>{article.excerpt}</p><small>{evidence.updated}</small></header>
...
function Footer({ lang }: { lang: Lang }) {
  ...
  return <footer className="site-footer">
    ...
    <p>{t.disclaimer}</p><span>spreadsheets-superbuy.net · 2026</span>
  </footer>;
}
...
export default function Site({ lang, route, articleSlug, routePath }: { lang: Lang; route: RouteKey | "article"; articleSlug?: ArticleSlug; routePath: string }) {
  ...
  return <main className="mint-site" lang={lang === "zh-cn" ? "zh-CN" : lang}><Header lang={lang} routePath={routePath} />{body}<Footer lang={lang} /></main>;
}
